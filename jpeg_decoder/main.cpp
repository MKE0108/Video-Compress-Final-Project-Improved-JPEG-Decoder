...
const int DC = 0;
const int AC = 1;
// map-based Huffman storage (legacy, kept for reference)
// std::map<std::pair<unsigned char, unsigned int>, unsigned char> huffTable[2][2];

struct HuffTableEntry {
    unsigned int startCode[17];
    unsigned int endCode[17];
    unsigned int count[17];
    std::vector<unsigned char> symbols[17];
};

HuffTableEntry huffTable[2][2];
...
void readDHT(FILE *f) {
    ...
    while (len > 0) {
        ...
        printf("\n");
        auto &table = huffTable[DCorAC][id];
        unsigned int codeVal = 0;
        for (int len = 1; len <= 16; len++) {
            table.startCode[len] = codeVal;
            table.count[len] = a[len - 1]; //有幾個child
            if (table.count[len] > 0) {
                table.endCode[len] = table.startCode[len] + table.count[len] - 1;
                table.symbols[len].assign(table.count[len], 0);
            } else {
                table.endCode[len] = 0;
                table.symbols[len].clear();
            }
            codeVal = (codeVal + table.count[len]) << 1; //進入下一層
        }
        for (int len = 1; len <= 16; len++) {
            for (unsigned int j = 0; j < table.count[len]; j++) {
                unsigned char v;
                fread(&v, 1, 1, f);
                table.symbols[len][j] = v;
                printf("len %d code %u: %d\n", len, table.startCode[len] + j, v);
            }
        }
...
unsigned char matchHuff(FILE *f, unsigned char number, unsigned char ACorDC) {
    ...
    auto &table = huffTable[ACorDC][number];
    while (true) {
        length++;
        codeVal = (codeVal << 1) | (unsigned int)getBit(f);
        if (length > 16) {
        ...
        if (codeVal < table.startCode[length]) {
            continue;
        }
        if (codeVal > table.endCode[length]) {
            continue;
        }
    ...
    while (c == 0xFF) {
        ...
        switch (c) {
            case SOI_MARKER:
                ...
                break;
            case APP0_MARKER: //0xE0 ~ 0xEF all will redirect to readAPP
            case 0xE1:
            case 0xE2:
            case 0xE3:
            case 0xE4:
